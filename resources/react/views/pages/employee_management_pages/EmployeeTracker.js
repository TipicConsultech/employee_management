...
function EmployeeCheckInOut() {
    ...
    const [status, setStatus] = useState({ checkIn: false, checkOut: false, company_gps: '', tolerance: '' });
    ...
    }, [showToast, t, trackerId]); // Include trackerId to ensure stability
    ...
    const handleAction = useCallback(async (type) => {
        ...
        try {
            ...
            if (response && (response.tracker || response.id || response.tracker_id || response.trackerId || response.success || response.message === 'success')) {
                const successMessage = type === 'checkin' ? t('MSG.checkInSuccess') || 'Check-in successful' : t('MSG.checkOutSuccess') || 'Check-out successful';
    ...
    }, [status, getCurrentLocationFresh, calculateDistance, showToast, t, fetchEmployeeStatus, trackerId]);
